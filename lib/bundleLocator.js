...
BundleLocator.prototype = {
    ...
    reportPluginMetrics: function () {
        console.log('------------------------ PLUGIN METRICS ------------------------');
        this._plugins.forEach(function(plugin) {
            if (! plugin.metrics.total.cnt) {
                // nothing to report
                return;
            }
            console.log(JSON.stringify(plugin.plugin.describe));
            var names = Object.keys(plugin.metrics);
            names.sort(function(a, b) {
                var diff = plugin.metrics[a].sum - plugin.metrics[b].sum;
                if (0 === diff) {
                    // sort alphabetically for things that took the same amount of time
                    return a - b;
                }
                return diff;
            });
            names.forEach(function(name) {
                console.log('    ' + plugin.metrics[name].sum + ' ' + plugin.metrics[name].cnt + ' ' + name);
            });
            console.log();
        });
    },
    ...
    _processBundleUpdates: function () {
        ...
        Object.keys(this._bundleUpdates).forEach(function (bundleName) {
            var evt = self._bundleUpdates[bundleName],
                pluginPromises = [],
                metricName = [EVENT_BUNDLE_UPDATED, bundleName].join(' ');
            ...
            self._plugins.forEach(function (plugin) {
                ...
                if (plugin.plugin[EVENT_BUNDLE_UPDATED]) {
                    pluginPromise = new Promise(function (resolve) {
                        plugin.metrics.total.cnt += 1;
                        if (! plugin.metrics[metricName]) {
                            plugin.metrics[metricName] = { cnt: 0, sum: 0 };
                        }
                        plugin.metrics[metricName].cnt += 1;
                        t0 = Date.now();
                        resolve(plugin.plugin[EVENT_BUNDLE_UPDATED](evt, self._pluginAPI));
                    });
                    pluginPromises.push(pluginPromise.then(function() {
                        var time = (Date.now() - t0);
                        plugin.metrics.total.sum += time;
                        plugin.metrics[metricName].sum += time;
                    }));
                }
            });
    ...
    _onFile: function (res, ruleset, eventType) {
        var self = this,
            ...
            promises = [],
            metricName = [fileEvent, res.bundleName, res.relativePath].join(' ');
        ...
        this._plugins.forEach(function (plugin) {
            ...
            if (self._filterResource(res, plugin.filter) && plugin.plugin[fileEvent]) {
                ...
                promises.push(pluginPromise.then(function() {
                    var time = (Date.now() - t0);
                    plugin.metrics.total.sum += time;
                    plugin.metrics[metricName].sum += time;
                }));
    ...
    _onResource: function (res, eventType) {
        var self = this,
            ...
            name = res.name,
            promises = [],
            metricName = [resourceEvent, res.bundleName, res.relativePath].join(' ');
